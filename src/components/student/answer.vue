        ...
        <div class="left" v-if="slider_flag">
          ...
          <div class="l-bottom">
            <div class="item">
              <p>选择题部分</p>
              <ul>
                <li v-for="(list, index1) in topic['multi_question']" :key="index1">
                  <a
                    href="javascript:;"
                    @click="change(index1)"
                    :class="{
                      border: index == index1 && currentType == 1,
                      bg: bg_flag && topic['multi_question'][index1].isClick == true,
                    }"
                  >
                    <span
                      :class="{ mark: topic['multi_question'][index1].isMark == true }"
                    ></span>
                    {{ index1 + 1 }}
                  </a>
                </li>
            ...
            <div class="item">
              <p>填空题部分</p>
              <ul>
                <li v-for="(list, index2) in topic['fill_question']" :key="index2">
                  <a
                    href="javascript:;"
                    @click="fill(index2)"
                    :class="{
                    ...
                    ><span
                      :class="{ mark: topic['fill_question'][index2].isMark == true }"
                    ></span
            ...
            <div class="item">
              <p>判断题部分</p>
              <ul>
                <li v-for="(list, index3) in topic['judge_question']" :key="index3">
                  <a
                    href="javascript:;"
                    @click="judge(index3)"
                    :class="{
                      border: index == index3 && currentType == 3,
                      bg: bg_flag && topic['judge_question'][index3].isClick == true,
                    }"
                    ><span
                      :class="{ mark: topic['judge_question'][index3].isMark == true }"
                    ></span
            ...
            <div class="fill" v-if="currentType == 2">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确姿势：</el-tag
                    ><span class="right">{{ topic['fill_question'][index].answer }}</span>
                  </li>
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>
                    {{
                      topic['fill_question'][index].analysis == null
                        ? "暂无解析"
                        : topic['fill_question'][index].analysis
                    }}
                  </li>
                </ul>
              </div>
            ...
            <div class="judge" v-if="currentType == 3">
              ...
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                    <el-tag type="success">正确姿势：</el-tag
                    ><span class="right">{{ topic['judge_question'][index].answer }}</span>
                  </li>
                  <li><el-tag>题目解析：</el-tag></li>
                  <li>
                    {{
                      topic['j'][index].analysis == null
                        ? "暂无解析"
                        : topic['judge_question'][index].analysis
                    }}
                  </li>
                </ul>
              </div>
...
let father = [];
let len = this.topicCount[Number(type)]
    for(let i = 0; i < len; i++) { //根据填空题数量创建二维空数组存放每道题答案
            let children = [null,null,null,null]
            father.push(children)
          }
          ...
          let getNum=()=>{
            ...
            let num=0;
            for(let i=1;i<Object.keys(this.topic).length;i++){
              num+=this.topic[name].length
            }
            console.log(num)
            return num;
          }
...
let name =
          this.currentType == "1"
            ...
            ? "fill_question"
            : "judge_question";
            this.index=this.topic[name].length;
