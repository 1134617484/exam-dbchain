...
<template>
  <div class="exam">
    <el-table :data="pagination.records" border>
      ...
      <el-table-column prop="term" label="学期或班级" width="100"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="add(scope.row)" type="primary" size="small">增加题库</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  methods: {
    async getExamInfo() { //分页查询所有试卷信息
     ...
     // 先获取当前用户的地址
      ...
      let data = await this.$DBChain.Querier(this.appCode)
    ...
    add(row) { //增加题库
    console.log(row)
      this.$router.push({path:'/addAnswerChildren',query: {paperId: row.paper_id,subject:row.source}})
    }
  },
  computed: {
    appCode() {
