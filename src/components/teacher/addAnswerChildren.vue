        ...
        <section class="append">
          <ul>
            ...
            <li v-if="optionValue == '选择题'">
              <span>正确选项:</span>
              <el-select
                v-model="postChange.right_answer"
                placeholder="选择正确答案"
                class="w150"
              >
                <el-option
...
export default {
  data() {
    return {
      ...
      postPaper: {
        //考试管理表对应字段
        paper_id: null,
        question_type: null, // 试卷类型 1--选择题  2--填空题   3--判断题
        question_id: null,
      },
      queryData: {
        score: "",
  ...
  methods: {
    ...
    getParams() {
      ...
      this.postPaper.paper_id = paperId;
    },
    changeSubmit() {
      //选择题题库提交
      this.postChange.subject = this.subject;
      let data = { ...this.postChange, ...this.queryData,...{status:"1",type:"1"} };
      console.log(data);

      this.insertRow(data, "multi_question", "题库添加成功","1");
      ...
      //   }
      // }).then(() => {
      //   this.$axios(`${this.API}/api/multiQuestionId`).then(res => { //获取当前题目的questionId
    ...
    fillSubmit() {
      ...
      let data = { ...this.postFill, ...this.queryData ,...{status:"2",type:"1"} };
      console.log(data);
     ...
     * text 成功后的message
     */
    async insertRow(row, tableName, text,questionType) {
      ...
      that.$DBChain.insertRow(that.appCode, tableName, row, async () => {
        ...
        let question = await that.$DBChain
          .Querier(that.appCode)
          [tableName].own.val();
        question.reverse();
        ...
        that.postPaper.question_id = questionId;
        that.postPaper.question_type = questionType;
        // 将试题加入试题库，。再将试题id与试卷id关联在试卷管理表
        // 插入前调用canInsertRow以初步断定其是否能正常插入
        console.log(that.appCode, "paper_manage", that.postPaper);
        let isCanInsert2 = await that.$DBChain.canInsertRow(
          that.appCode,
          "paper_manage",
          that.postPaper
        );
        console.log(isCanInsert2);

        if (!isCanInsert2) {
          that.$store.commit("setIsLoding", false);
          return that.$message.error("暂时不能插入，请检查原因");
        }
        this.$DBChain.insertRow(
          that.appCode,
          "paper_manage",
          that.postPaper,
          () => {
